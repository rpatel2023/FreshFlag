...
### First Ubuntu regeneration checkpoint
...
- Analyzer then failed with **55 issues**, all but two caused by one inherited `lib/services/notification_service.dart` file that still imported/used the deliberately removed local-notification/timezone packages.
- The remaining two analyzer infos were an auth `BuildContext`-across-async-gap warning and deprecated `ButtonBar` usage.
